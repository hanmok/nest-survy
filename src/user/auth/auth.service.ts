...
export class AuthService {
	constructor(
		...
		@InjectRepository(AccessToken) private accessTokenRepo: Repository<AccessToken>, 
		...
		const newToken = this.accessTokenRepo.create({token: accessToken, user_id: userId}) 
	...
	async generateRefreshToken(userId: number) {
		...
		const newToken = this.refreshTokenRepo.create({token: refreshToken, user_id: userId})
	...
	async verifyRefreshToken(token: string) { 
		const validToken = await this.refreshTokenRepo.findOne({where: {token}})
		// 이거 넣어야함 ;; 
		// if (!validToken) return new UnauthorizedException();
		const refreshToken = this.jwtService.verify(token, {secret: '046e13dae9c744286aea80fc54f6f203b1a15e36'})
		// return { refreshToken: refreshToken['refreshToken'] }
		return validToken.user_id 
	}

	async verifyAccessToken(token: string) { 
		const accessToken = this.jwtService.verify(token, {secret: '046e13dae9c744286aea80fc54f6f203b1a15e36'})
	...
	async userHasToken(userId: number) { 
		const user1 = await this.refreshTokenRepo.find({where: {user_id: userId}})
		const user2 = await this.accessTokenRepo.find({where: {user_id: userId}})
